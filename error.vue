<template>
    <Body class="overflow-hidden gradient-radial bg-gradient-to-r from-primary-500 to-secondary-500">
        <div class="overflow-hidden">
            <div class="h-screen w-screen flex flex-col justify-center md:justify-end items-start">
                <div
                    ...
                    <div class="block md:hidden">
                        <h1 class="text-7xl xs:text-8xl text-accent-300 font-bold">{{ error.statusCode }}</h1>
                    </div>
                    <h2
                        class="uppercase text-surface-700 lg:text-surface-900 text-4xl sm:text-6xl md:text-8xl font-bold lg:mix-blend-overlay">
                        {{ error.message }}
                    </h2>
                    <p class="md:text-lg text-surface-50">We're sorry, but something went wrong. Please try again later or
        ...
        <div
            ...
            <h1 class="text-accent-200 text-center mt-0 uppercase font-bold md:rotate-[28deg] md:-skew-x-[25deg]"
                ref="glitchedHeader" :style="{ textShadow: longShadow }">
                {{ error.statusCode }}
            </h1>
            <div class="glitch-window absolute top-0 left-[-2px] w-full overflow-hidden bg-transparent" ref="glitchWindow">
            </div>
            <div class="shadow-window absolute top-[40px] left-[1px] w-full !bg-transparent overflow-hidden z-[-1] blur-lg"
                ref="shadowWindow"></div>
        </div>
    </Body>
</template>
