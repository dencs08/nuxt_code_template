...
export function useAuthentication() {
    const authService: IAuthenticationService = authenticationService('supabase');
    ...
    const updateEmail = (email: string, emailRedirectTo?: string): Promise<void> => updateAuthProfile({ email }, { emailRedirectTo });
  
    // Used when wanting to change the password with the current password verification
    const changeUserPassword = async (currentPassword: string, newPassword: string): Promise<any> => {
