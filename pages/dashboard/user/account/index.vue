                            ...
                            <template #default="{ getNode }">
                                <div class="grid grid-cols-2 gap-4 mb-4">
                                    ...
                                    <div class="col-span-2">
                                        <FormKit class="w-full" type='primeInputText' name='email'
                                            validation='required|email' placeholder='Email' v-model="userDetails.email"
...
const onProfileSave = async (data: FormData) => {
    ...
    if (initialUserDetails.email !== data.email) {
        await updateUserEmail(email, `${window.location.origin}/dashboard/account/confirm-email-change/`);
    }
