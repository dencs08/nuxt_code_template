<template>
    <div>
                                        ...
                                        <li>
                                            <ul role="list" class="-mx-2 space-y-1">
                                                <li v-for="item in navigation" :key="item.name">
                                                    <CardLink :to="item.href" :icon="item.icon">{{ item.name }}</CardLink>
                                                </li>
                                            </ul>
                                        </li>
                                        <li>
                                            ...
                                            <ul role="list" class="-mx-2 mt-2 space-y-1">
                                                <li v-for="team in teams" :key="team.name">
                                                    <CardLink :to="team.href">{{ team.name }}</CardLink>
                                                </li>
                                            </ul>
                                        </li>
                                        <li class="mt-auto">
                                            <CardLink to="DashboardSettings" icon="ic:round-settings">Settings</CardLink>
                                        </li>
                                    </ul>
                                </nav>
                            </div>
        ...
        <div class="hidden fixed top-20 lg:z-50 lg:flex lg:w-64 lg:flex-col h-full">
            <!-- Sidebar component, swap this element with another sidebar if you like -->
            <div class="flex grow flex-col gap-y-5 overflow-y-auto px-6 pb-4">
                <nav class="flex flex-1 flex-col">
                    <ul role="list" class="flex flex-1 flex-col gap-y-7">
                        ...
                        <li class="mt-auto mb-20">
                            <CardLink to="DashboardSettings" icon="ic:round-settings" :list="[
                                { name: 'Link 1', href: 'DashboardHome', icon: 'ic:round-settings' },
                                { name: 'Link 2', href: 'DashboardHome' }
                            ]">Settings</CardLink>
                        </li>
                    </ul>
                </nav>
...
const navigation = [
    { name: 'Dashboard', href: 'DashboardHome', icon: 'ic:round-home' },
    { name: 'Recruitments', href: 'DashboardRecruitments', icon: 'ic:round-business-center' },
    { name: 'Applicants', href: 'DashboardApplicants', icon: 'ic:round-people' },
    // { name: 'Quizes', href: 'DashboardQuizes', icon: 'ic:round-help-center' },
    // { name: 'Calendar (WIP)', href: 'DashboardCalendar', icon: 'ic:round-calendar-month' },
    // { name: 'Reports (WIP)', href: 'DashboardReports', icon: 'ic:round-data-saver-off' },
]

const teams = [
    { id: 1, name: 'Users', href: 'DashboardUsers' },
]
...
const handleItemClick = async (item) => {
    if (item.name === 'Sign out') {
        try {
            const response = await signOut({ callbackUrl: '/login' });
...
const userNavigation = computed(() => [
    { name: 'Your profile', to: { name: 'DashboardProfileIndex', params: { id: user.value.id } } },
    { name: 'Sign out' },
]);
</script>