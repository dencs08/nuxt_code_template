<template>
  <div @mouseenter="toggle(true)" @mouseleave="toggle(false)">
    ...
    <!--tooltip starts here-->
    <!-- <div v-if="list.length > 0 && !isActive" class="absolute right-0 -translate-y-1/2 z-[9999]" v-auto-animate
      @mouseenter="cancelHideTooltip" @mouseleave="startHideTooltip">
      <div v-if="toggleValue" class="absolute -bottom-3 left-0 bg-white shadow-lg min-w-[150px] rounded-md">
        <NuxtLink v-for="(item, index) in list" :key="index" :to="item.to"
