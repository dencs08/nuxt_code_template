<template>
    <DataTable ref="dt" :value="items" v-model:selection="selected" v-model:editingRows="editingRows"
        v-model:filters="filters" dataKey="id" tableStyle="min-width: 50rem;" editMode="row" @row-edit-save="onRowEditSave"
        :rows="rows" :globalFilterFields="globalFilterFields" size="small">

        <template #header>
            <div class="flex justify-between items-center">
                <div v-auto-animate v-if="hasAccess" class="flex gap-2 w-full items-center">
                    <UserCreator />
                    <Button size="small" label="Delete" severity="danger" @click=""
                ...
                <div class="flex justify-between gap-3" :class="!hasAccess ? 'w-full' : ''">
                    ...
                    <div class="grid place-content-center">
                        <Button size="small" class="pi pi-external-link text-surface-700 dark:text-surface-100"
                            v-tooltip.left="'Export table to CSV'" @click="exportCSV()" />
                    </div>
                    <div class="grid place-content-center">
                        <Button size="small" v-tooltip.left="'Refresh table'" @click=""
        ...
        <Column v-if="selectable" :selectionMode="selectable" headerStyle="width: 3rem"></Column>

        <Column v-for="column in columns" :key="column.field" :header="column.header" v-bind="column.props"
        ...
        <Column v-if="editable" :rowEditor="true" style="width: 10%; min-width: 6rem" bodyStyle="text-align:center">
        </Column>
    </DataTable>
</template>
    ...
    return props.items
        .map((item, index) => {
            ...
            const changedValues = Object.entries(item)
                ...
                .map(([key, value]) => ({ key, oldValue: originalItem[key], newValue: value }))
